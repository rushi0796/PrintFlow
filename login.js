...
// ======================================================
// RAZORPAY REVIEWER / DEMO LOGIN HANDLER
// ======================================================

const toggleReviewerLoginBtn = document.getElementById("toggleReviewerLoginBtn");
const reviewerLoginForm = document.getElementById("reviewerLoginForm");
const reviewerMobileInput = document.getElementById("reviewerMobile");
const reviewerAccessKeyInput = document.getElementById("reviewerAccessKey");
const reviewerErrorMsg = document.getElementById("reviewerErrorMsg");
const reviewerLoginSubmitBtn = document.getElementById("reviewerLoginSubmitBtn");

if (toggleReviewerLoginBtn && reviewerLoginForm) {
    toggleReviewerLoginBtn.addEventListener("click", function () {
        const isHidden = reviewerLoginForm.style.display === "none" || !reviewerLoginForm.style.display;
        reviewerLoginForm.style.display = isHidden ? "block" : "none";
        if (isHidden && reviewerAccessKeyInput) {
            reviewerAccessKeyInput.focus();
        }
    });
}

if (reviewerLoginForm) {
    reviewerLoginForm.addEventListener("submit", async function (e) {
        e.preventDefault();
        if (reviewerErrorMsg) reviewerErrorMsg.style.display = "none";

        const mobileVal = reviewerMobileInput ? reviewerMobileInput.value.trim() : "9999999999";
        const keyVal = reviewerAccessKeyInput ? reviewerAccessKeyInput.value.trim() : "";

        if (!keyVal) {
            if (reviewerErrorMsg) {
                reviewerErrorMsg.textContent = "⚠️ Please enter the Reviewer Access Key.";
                reviewerErrorMsg.style.display = "block";
            }
            return;
        }

        if (reviewerLoginSubmitBtn) {
            reviewerLoginSubmitBtn.disabled = true;
            reviewerLoginSubmitBtn.textContent = "Authenticating...";
        }

        try {
            const isLocal = ["localhost", "127.0.0.1"].includes(window.location.hostname) || window.location.protocol === "file:";
            const apiEndpoint = isLocal ? "http://127.0.0.1:8000/api/reviewer-login" : "/api/reviewer-login";
            ...
            try {
                ...
                throw new Error("Server returned non-JSON response. Ensure backend server is running.");
            }

            if (res.ok && data && data.status === "success") {
                localStorage.setItem("mobileNumber", data.mobile || "+919999999999");
                localStorage.setItem("isReviewer", "true");
                window.location.href = "home.html";
            } else {
                const errMsg = (data && data.detail) ? data.detail : "Invalid Reviewer Credentials";
                if (reviewerErrorMsg) {
                    reviewerErrorMsg.textContent = `❌ ${errMsg}`;
                    reviewerErrorMsg.style.display = "block";
                }
            }
        } catch (err) {
            ...
            if (reviewerErrorMsg) {
                reviewerErrorMsg.textContent = `⚠️ Error: ${err.message || "Connection failed"}`;
                reviewerErrorMsg.style.display = "block";
            }
        } finally {
            if (reviewerLoginSubmitBtn) {
                reviewerLoginSubmitBtn.disabled = false;
                reviewerLoginSubmitBtn.textContent = "Login as Reviewer ➔";
            }
        }
    });
}